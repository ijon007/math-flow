...
export const POST = Webhooks({
  ...
  onSubscriptionCreated: async (payload) => {
    ...
    try {
      ...
      const userId = customer.metadata?.clerkUserId || customer.metadata?.userId || customer.id;
      
      console.log('Webhook - Customer metadata:', customer.metadata);
      console.log('Webhook - Customer ID:', customer.id);
      console.log('Webhook - Clerk user ID from metadata:', customer.metadata?.clerkUserId);
      ...
      await convex.mutation(api.subscriptions.createOrUpdateSubscription, {
        ...
        plan: getPlanFromInterval(subscription.prices[0]?.recurringInterval || "month"),
