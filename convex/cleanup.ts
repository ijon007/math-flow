...
export const cleanupOrphanedData = mutation({
  ...
  handler: async (ctx) => {
    ...
    const orphanedMessages = allMessages.filter(m => !threadIds.has(m.threadId));
    ...
    const orphanedFlashcards = allFlashcards.filter(f => !threadIds.has(f.threadId));
    ...
    const orphanedStepByStep = allStepByStep.filter(s => !threadIds.has(s.threadId));
    
    console.log(`Found ${orphanedStepByStep.length} orphaned step-by-step solutions`);
    ...
    const orphanedBookmarks = allBookmarks.filter(b => !threadIds.has(b.threadId));
